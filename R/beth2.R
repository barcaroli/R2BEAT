                      ...
                      epsilon = 10^(-11), minnumstrat = 2, maxiter = 200, maxiter1 = 25) 
{
  ...
  names(param_alloc) = c("p_epsilon", "p_minnumstrat", "p_maxiter", 
                         "p_maxiter1", "p_epsilon1", "p_diffx", "p_iterx", "p_mmdiff_deft", 
                         "p_maxi", "num_stages")
  nvar = ncol(errors) - 1
  if (!is.null(effst)) {
  ...
  newDeft <- function(des_file, stratif, errors, rho, psu_file, 
    ...
    while (diffx > epsilon1 && iterx < maxi && mmdiff_deft > 
      ...
      PSU_NSR <- round(sum(aggregate(psu_file1$POPNAR/psu_file1$SOGLIA, 
  ...
  out <- list(iterations = iterations, file_strata = file_strata, 
              alloc = alloc, planned = planned, expected = expected, 
